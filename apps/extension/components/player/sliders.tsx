...
export function Time({ thumbnails }: TimeSliderProps) {
  ...
  return (
    ...
    >
      ...
      <Slider.Thumb
        aria-label="Current Time"
        className="block h-[15px] w-[15px] rounded-full border border-[#cacaca] bg-white outline-none ring-white/40 transition-opacity group-hocus:opacity-100 focus:opacity-100 focus:ring-4 will-change-[left]"
      />
      ...
      > */}
      <div
        className="flex flex-col items-center absolute data-[visible]:opacity-100 transition-opacity duration-200 will-change-[left] pointer-events-none"
        ref={previewRef}
      >
        {thumbnails ? (
          <Thumbnail.Root
